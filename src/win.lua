...
local function initWin()
  ...
  data.sprite = images.getImage("victorySheet")
  data.w = 1317
  data.h = 288
  data.x = screen.getWidth()/2
  data.y = screen.getHeight()/2 - 50

  data.animation = animationUpDown.new(data.sprite, 34, 18, 0, 0, data.w, data.h)
end
...
local function updateWinAnim(dt)
  if (data.animation.currentFrame < 34) then
    animationUpDown.update(data.animation, dt)
  else
    data.animation.currentFrame = 34
  end
end
